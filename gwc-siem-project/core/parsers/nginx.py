...
NGINX_LOG_REGEX = re.compile(r'^(?P<ip>\S+) \S+ \S+ \[(?P<timestamp>.*?)\] "(?P<method>\S+) (?P<url>\S+) .*?" (?P<status>\d{3}) (?P<size>\S+) ".*?" ".*?"')

def parse_nginx_log(log_line: str) -> dict | None:
    """
    Parses a single line of an Nginx access log.
    """
    match = NGINX_LOG_REGEX.match(log_line)
    if not match:
        return None
